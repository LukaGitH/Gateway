...
function normalizeNodeId(nodeId) {
  if (nodeId === undefined || nodeId === null || nodeId === '') return undefined;
  if (typeof nodeId === 'string' && nodeId.trim() === '') return undefined;
  if (typeof isNumeric === 'function' && isNumeric(nodeId)) return parseInt(nodeId);
  return nodeId;
}

function normalizeMultiGraphSelection(selection, fallbackNodeId) {
  ...
  if (typeof selection === 'object') {
    ...
    if (parts.length === 2) {
      nodeId = normalizeNodeId(parts[0]);
      metricKey = parts[1];
    }
    else metricKey = selection;
  }
...
io.sockets.on('connection', function (socket) {
    ...
    db.find({ _id : nodeId }, function (err, entries) {
      if (entries.length == 1)
      {
        ...
        metricsList.forEach(function(metricSelection) {
          ...
          if (newGraph.find(function(entry) { return (entry.nodeId + '::' + entry.metricKey) == selectionKey; })) return;
          newGraph.push(normalized);
        });

        if (newGraph.length < 2) {
          socket.emit('LOG', 'ADDNODEGRAPH FAIL: select at least 2 valid metrics');
          return;
        }
  ...
  socket.on('GETMULTIGRAPHDATA', function (nodeId, multiGraphId, start, end, exportMode) {
    db.find({ _id : nodeId }, function (err, entries) {
      if (entries.length == 1)
      {
        ...
        var multiGraphSelections = dbNode.multiGraphs[multiGraphId] || [];
        var normalizedSelections = multiGraphSelections.map(function(selection) {
          return normalizeMultiGraphSelection(selection, dbNode._id);
        }).filter(function(selection) {
          return selection && selection.metricKey;
        });

        if (!normalizedSelections.length) {
          socket.emit(exportMode ? 'EXPORTMULTIGRAPHDATAREADY' : 'MULTIGRAPHDATAREADY', []);
          return;
        }

        var responseSeries = new Array(normalizedSelections.length);
        var pending = normalizedSelections.length;
        var respond = function() {
          var filtered = responseSeries.filter(function(series) { return series != null; });
          ...
          if (selection.nodeId == dbNode._id || selection.nodeId === undefined) handleNode(dbNode);
          else {
            db.find({ _id : selection.nodeId }, function (err, targetEntries) {
              handleNode(targetEntries && targetEntries.length ? targetEntries[0] : null);
            });
          }
        });
      }
  ...
  socket.on('GETCUSTOMMULTIGRAPHDATA', function (selectionArray, start, end, exportMode) {
    if (!Array.isArray(selectionArray) || selectionArray.length == 0) {
      socket.emit(exportMode ? 'EXPORTMULTIGRAPHDATAREADY' : 'CUSTOMMULTIGRAPHDATAREADY', []);
      return;
    }
    var normalizedSelections = selectionArray.map(function(selection) {
      return normalizeMultiGraphSelection(selection);
    }).filter(function(selection) {
      return selection && selection.nodeId != undefined && selection.metricKey;
    });
    if (!normalizedSelections.length) {
      socket.emit(exportMode ? 'EXPORTMULTIGRAPHDATAREADY' : 'CUSTOMMULTIGRAPHDATAREADY', []);
      return;
    ...
    var respond = function() {
      var filtered = responseSeries.filter(function(series) { return series != null; });
      socket.emit(exportMode ? 'EXPORTMULTIGRAPHDATAREADY' : 'CUSTOMMULTIGRAPHDATAREADY', filtered);
    };
    normalizedSelections.forEach(function(selection, index) {
      db.find({ _id : selection.nodeId }, function (err, entries) {
        var targetNode = entries && entries.length ? entries[0] : null;
        if (targetNode && targetNode.metrics && targetNode.metrics[selection.metricKey] && targetNode.metrics[selection.metricKey].graph == 1) {
          var graphData = getGraphData(targetNode._id, selection.metricKey, start, end, exportMode);
          graphData.options = graphData.options || {};
          graphData.options.nodeId = targetNode._id;
